...
DEFINE_int32(value_size_max, 1024, "Max size of random value");
...
DEFINE_bool(print_keys, false,
            "If set true, the mixgraph workload will printout the keys");
DEFINE_double(
...
class RandomGenerator {
 ...
 public:
  ...
  RandomGenerator(int target_size) {
    auto max_value_size = target_size;
    dist_.reset(new FixedDistribution(target_size));
    max_value_size = target_size;
    // We use a limited amount of data over and over again and ensure
    // that it is larger than the compression window (32KB), and also
    // large enough to serve all typical value sizes we want to write.
    Random rnd(301);
...
static uint64_t ParseThePathSize(std::string path_size) {
  ...
  double num = 0;
  auto num_begin =
      std::sregex_iterator(path_size.begin(), path_size.end(), num_regex);
  ...
  std::string unit_str =
      path_size.substr(num_length, (path_size.length() - num_length));
...
static DbPath ParseSingleDbpath(std::string* db_path) {
  ...
  auto path_begin =
      std::sregex_iterator(db_path->begin(), db_path->end(), path_regex);
  ...
  auto path_size_begin =
      std::sregex_iterator(db_path->begin(), db_path->end(), size_regex);
...
// Current the following isn't equivalent to OS_LINUX.
  ...
  inline void ParseStringToChangePoint(std::string origin, ChangePoint* point) {
    std::vector<std::string> values =
        ROCKSDB_NAMESPACE::StringSplit(origin, ',');
    point->opt = values[0];
    point->value = values[1];
    point->change_timing = std::stoi(values[2]);
  }

  Status ParseChangePointFlag(std::string* str) {
    Status s;
    config_change_points = std::vector<ChangePoint>();

    std::regex change_point_regex("(\\w)+,[0-9]+,[0-9]+");
    auto words_begin =
        std::sregex_iterator(str->begin(), str->end(), change_point_regex);
    auto words_end = std::sregex_iterator();
    for (std::sregex_iterator i = words_begin; i != words_end; ++i) {
      std::smatch match = *i;
      //      std::cout << "  " << match_str << '\n';
      ChangePoint temp;
      ParseStringToChangePoint(match.str(), &temp);
      config_change_points.push_back(temp);
    }
    return s.OK();
  }
 ...
 public:
  ...
  void Run() {
    ...
    // test the change point
    ParseChangePointFlag(&FLAGS_change_points);
...
// optimization, so skip the check of the max level.
  ...
  void BGMixgraph(ThreadState* thread) {
    if (thread->tid > 0) {
      MixGraph(thread);
      std::cout << "end_mixgraph" << std::endl;
    } else {
      BurstWritten(thread, kWrite);
      std::cout << "end_burst_written" << std::endl;
    }
  }
